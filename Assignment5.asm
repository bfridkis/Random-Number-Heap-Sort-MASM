...
displayList PROC
	...
	push	esi
	push	edi
	sub		esp, 5										;[epb - 4] = number of complete rows - BYTE
...
IncrementRowNumber:
	inc		BYTE PTR[ebp - 6]							;Incremments output row number
...
RightChildSmallest:
	mov		[ebp - 32], cx					;Moves value of smallest child (left) into local variable
	mov		eax, [ebp + 8]					;Multiply position index by 2... 
	mul		bl								
	add		eax, [ebp + 16]					;...and add to array OFFSET to get address of array at index
	mov		ax, WORD PTR[eax]				;Move the value of index position into eax by dereferencing its address
